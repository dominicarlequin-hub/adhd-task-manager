...
import { Plus, ListPlus, Check, RotateCcw, Sparkles, Pencil, Trash2, Wand2 } from "lucide-react";
...
export default function App() {
  ...
  const [activeList, setActiveList] = useState<ListName | "All">("All");
  ...
  return (
    <div className="min-h-screen bg-cream text-ink font-sans">
      <div className="mx-auto max-w-md min-h-screen flex flex-col px-6 pt-14 pb-8">
        ...
        {tasks.length > 0 && (
          <div className="flex gap-2 mb-6 overflow-x-auto -mx-1 px-1 pb-1">
            {(["All", "Home", "Work", "Personal"] as const).map((tab) => {
              const selected = activeList === tab;
              const count =
                tab === "All"
                  ? tasks.filter((t) => !t.done).length
                  : tasks.filter((t) => !t.done && t.list === tab).length;
              return (
                <button
                  key={tab}
                  onClick={() => setActiveList(tab)}
                  className={`shrink-0 flex items-center gap-1.5 rounded-2xl px-3.5 py-2 text-[13px] font-bold transition-all ${
                    selected
                ...
                >
                  {tab !== "All" && (
                    <span className={`w-2 h-2 rounded-full ${LIST_DOT[tab]}`} />
                  )}
                  {tab}
      ...
      {addOpen && (
        <QuickAddSheet
          defaultList={activeList === "All" ? "Home" : activeList}
...
function QuickAddSheet({
  ...
  const [list, setList] = useState<ListName>(defaultList);
  ...
  async function handleCleanUp() {
    ...
    try {
      ...
      if (res.ok && data.cleaned) {
        setName(data.cleaned);
      } else {
        setCleanError(true);
      }
    } catch {
    ...
    <div className="fixed inset-0 z-50 bg-black/30 flex items-end sm:items-center sm:justify-center">
      <div className="w-full sm:max-w-md bg-cream rounded-t-3xl sm:rounded-3xl px-6 pt-6 pb-8 max-h-[90vh] overflow-y-auto">
        ...
        <button
          ...
          disabled={!name.trim() || cleaning}
          className="mt-2.5 mb-1 inline-flex items-center gap-1.5 rounded-2xl bg-white px-3.5 py-2 text-[12px] font-bold text-terracotta shadow-[0_3px_0_#DDD2C2] disabled:opacity-40 active:translate-y-[2px] active:shadow-none transition"
        >
          <Wand2 size={13} className={cleaning ? "animate-pulse" : ""} />
          {cleaning ? "Cleaning up..." : "Clean up with AI"}
        </button>
        {cleanError && (
          <p className="text-xs text-red-500 mt-1 mb-1">Couldn't clean that up — try again or just save as-is.</p>
        )}
